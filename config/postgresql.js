...
// file: PostgreSQLManager.mjs
// file: PostgreSQLManager.js
...
class PostgreSQLManager {
  ...
  async connect() {
    ...
    try {
      const connectionString = process.env.DATABASE_URL || 
        `postgres://${process.env.PG_USER || 'postgres'}:${process.env.PG_PASSWORD || ''}` +
        `@${process.env.PG_HOST || 'localhost'}:${process.env.PG_PORT || 5432}/${process.env.PG_DATABASE || 'gps_tracker'}`;
    ...
    try {
      const result = await this.sql`
        INSERT INTO locations (
          ...
          ${locationData.ignition || false},
          ${locationData.engineOn || false},
          ${locationData.battery || null},
          ${locationData.power || null},
          ${locationData.rssi || null},
          ${locationData.protocol},
          ${locationData.alarms || []},
          ${locationData.attributes || {}}
        ) RETURNING id
      ...
      logger.error('Error inserting location:', error);
      throw error;
    }
  }
        ...
        for (const row of rows) {
          await sql`
            INSERT INTO locations (
              ...
              ${row[0]}, ${row[1]}, POINT(${row[2].slice(1, -1)}), ${row[3]}, ${row[4]}, ${row[5]},
  ...
  async findNearbyDevices(latitude, longitude, radiusKm = 10) {
    return this.sql`
      SELECT DISTINCT ON (device_id)
        ...
        speed
      FROM locations
      WHERE location <-> POINT(${longitude}, ${latitude}) < ${radiusKm / 111.0}
        AND timestamp > NOW() - INTERVAL '1 hour'
      ORDER BY device_id, timestamp DESC
    `;
  }
  ...
  async archiveOldData(daysToKeep = 90) {
    ...
    const result = await this.sql.begin(async sql => {
      await sql`
        INSERT INTO locations_archive
        SELECT * FROM locations WHERE timestamp < ${cutoffDate}
      `;

      const del = await sql`
        DELETE FROM locations WHERE timestamp < ${cutoffDate}
      `;
      return del;
    });

    logger.info(`Archived ${result.count || 0} location records`);
  ...
  async cleanup(daysToKeep = 365) {
    const cutoffDate = new Date(Date.now() - daysToKeep * 24 * 60 * 60 * 1000);
    const result = await this.sql`
      DELETE FROM locations_archive WHERE timestamp < ${cutoffDate}
    `;
    logger.info(`Cleaned up ${result.count || 0} archived records`);
    return result.count || 0;
  }
